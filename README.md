...
4. После деплоя — смоук: `GET /api/health` должен вернуть `200`, поле `gitSha`
   совпадает с только что запушенным коммитом. Состояние базы показывается
   только авторизованному: `curl -H "Authorization: Bearer $CRON_SECRET"
   <APP_URL>/api/health` → `checks.database.ok = true`.
...
- `/api/health` **всегда** отвечает `200`; реальное состояние — в теле (`ok`,
  `checks.database`). Проба базы выполняется только по `Bearer CRON_SECRET` или
  из сессии владельца — роут публичный, и `SELECT 1` каждому встречному
  позволял бы выкачать общий лимит компьюта Neon. Хост и порт базы в теле не
  появляются никогда: только категория отказа.
...
> **Первый запуск в прод — пошагово: [`docs/DEPLOY.md`](docs/DEPLOY.md).**
> Там же список переменных окружения целиком и таблица диагностики.
