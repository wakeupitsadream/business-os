 ...
 * и «не сошлось»: операция 1 июля в 02:00 МСК по UTC ещё июньская, и месячный
 * итог, посчитанный по UTC, отличается от банковского.
 *
 * Москва — UTC+3 круглый год (перевода часов нет с 2014-го), поэтому начало
 * месяца по владельцу — это 21:00 предыдущего дня по UTC.
 */

const iso = (d: Date) => d.toISOString();

describe("месяц по времени владельца", () => {
  it("режется по московской полуночи, а не по UTC", () => {
    const r = resolvePeriod("month", new Date("2026-07-15T12:00:00Z"));
    expect(iso(r.start)).toBe("2026-06-30T21:00:00.000Z");
    expect(iso(r.end)).toBe("2026-07-31T21:00:00.000Z");
  });

  it("операция 1 июля в 02:00 МСК попадает в июль", () => {
    const r = resolvePeriod("month", new Date("2026-07-15T12:00:00Z"));
    const tx = new Date("2026-06-30T23:00:00Z"); // 02:00 МСК 1 июля
...
describe("прошлый месяц", () => {
  it("в январе уходит в декабрь прошлого года", () => {
    ...
    expect(iso(r.start)).toBe("2025-11-30T21:00:00.000Z");
    expect(iso(r.end)).toBe("2025-12-31T21:00:00.000Z");
  });
...
describe("неделя", () => {
  it("начинается с понедельника", () => {
    ...
    expect(iso(r.start)).toBe("2026-07-19T21:00:00.000Z"); // 20 июля 00:00 МСК
    expect(iso(r.end)).toBe("2026-07-26T21:00:00.000Z");
  });
...
describe("сутки", () => {
  ...
  it("после московской полуночи «сегодня» — уже новый день", () => {
    // 00:30 МСК 16 июля = 21:30 UTC 15 июля.
    const r = resolvePeriod("today", new Date("2026-07-15T21:30:00Z"));
    expect(iso(r.start)).toBe("2026-07-15T21:00:00.000Z");
  });
});

describe("год и квартал", () => {
  it("год режется по московской полуночи 1 января", () => {
    const r = resolvePeriod("year", new Date("2026-07-15T12:00:00Z"));
    expect(iso(r.start)).toBe("2025-12-31T21:00:00.000Z");
  ...
  it("квартал — три месяца, включая текущий", () => {
    ...
    expect(iso(r.start)).toBe("2026-04-30T21:00:00.000Z"); // 1 мая по МСК
    expect(iso(r.end)).toBe("2026-07-31T21:00:00.000Z");
  });
});
