...
describe("произвольный диапазон", () => {
  ...
  it("начальная дата тоже режется по суткам владельца", () => {
    // «2026-07-01» разбирается как полночь UTC, а у владельца (UTC+5) эти
    // сутки начались пятью часами раньше. Без приведения ночные операции
    // первого дня выпадали из отчёта — молча, просто суммой меньше банковской.
    const r = explicitRange("2026-07-01", "2026-07-31");
    expect(r).not.toBeNull();
    expect(iso(r!.start)).toBe("2026-06-30T19:00:00.000Z");
  });

  it("операция в 02:00 местного первого числа попадает в диапазон", () => {
    // Тот же случай, что ломал месячные итоги: 1 июля 02:00 по владельцу — это
    // 30 июня 21:00 UTC.
    const r = explicitRange("2026-07-01", "2026-07-31")!;
    const nightTaxi = new Date("2026-06-30T21:00:00.000Z");
    expect(nightTaxi >= r.start && nightTaxi < r.end).toBe(true);
  });
