...
describe("произвольный диапазон", () => {
  ...
  it("начальная дата тоже режется по владельцу, а не по UTC", () => {
    // «2026-07-01» само по себе — полночь UTC, то есть пять утра
    // екатеринбургского. Ночная операция первого числа выпадала бы из отчёта,
    // хотя владелец её в этот день и сделал.
    const r = explicitRange("2026-07-01", "2026-07-31");
    expect(iso(r!.start)).toBe("2026-06-30T19:00:00.000Z");
  });

  it("операция в 02:00 первого числа попадает в диапазон", () => {
    // Та самая проверка «сошлось с выпиской»: без неё расход за месяц меньше
    // банковского ровно на ночные операции первого дня.
    const r = explicitRange("2026-07-01", "2026-07-31")!;
    const nightly = new Date("2026-06-30T21:00:00Z"); // 02:00 местного 1 июля
    expect(nightly >= r.start && nightly < r.end).toBe(true);
  });

  it("однодневный диапазон — это ровно сутки владельца", () => {
    const r = explicitRange("2026-07-05", "2026-07-05")!;
    expect(iso(r.start)).toBe("2026-07-04T19:00:00.000Z");
    expect(iso(r.end)).toBe("2026-07-05T19:00:00.000Z");
