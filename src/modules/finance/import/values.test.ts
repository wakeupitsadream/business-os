...
describe("даты", () => {
  it("дата с временем читается как МЕСТНАЯ, а не как UTC", () => {
    // Банк печатает 14:23 по времени владельца (UTC+5) — это 09:23 UTC.
    // Прочитать строку как UTC значит сдвинуть операцию на пять часов.
    const d = parseStatementDate("05.07.2026 14:23:05");
    expect(d?.toISOString()).toBe("2026-07-05T09:23:05.000Z");
  });

  it("дата без времени — местная полночь", () => {
    const d = parseStatementDate("05.07.2026");
    expect(d?.toISOString()).toBe("2026-07-04T19:00:00.000Z");
  ...
  it("операция первого числа ночью остаётся в своём месяце", () => {
    ...
    // 01.08 01:00 местного по UTC — это ещё 31 июля.
    const d = parseStatementDate("01.08.2026 01:00:00");
    expect(d?.toISOString()).toBe("2026-07-31T20:00:00.000Z");
    expect(d && dayKey(d)).toBe("2026-08-01");
  });

  it("понимает ISO как запасной формат", () => {
    expect(parseStatementDate("2026-07-05")?.toISOString()).toBe("2026-07-04T19:00:00.000Z");
  });
