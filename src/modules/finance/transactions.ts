 ...
 * описание.
 *
 * **Ключ НЕ уникален.** Две чашки кофе по 200 ₽ в один день на одном счёте —
 * законная пара операций, и различить их нечем: у них совпадает всё, из чего
 * строится ключ, а времени банк в выписке даёт не всегда. Пока на пару
 * (счёт, ключ) стоял уникальный индекс, вторая такая строка молча не
 * импортировалась: в предпросмотре она даже не показывалась, владелец видел
 * только «Дублей: 1» и не мог узнать, что за этим числом.
 *
 * Инвариант «повторная загрузка того же файла даёт ноль новых строк» держится
 * не индексом, а счётом: `classifyRows` смотрит, сколько операций с этим
 * ключом уже лежит в базе, и ровно столько ПЕРВЫХ строк файла помечает
 * дублями. Остальные — новые. Отсюда же и правильное поведение на файле из
 * трёх одинаковых строк, когда в базе их две: дублями станут две, третья
 * запишется.
 *
 * Ручному вводу ключ не ставится вовсе — там дедупить нечего.
 *
 * День режется по поясу владельца, как и все периоды системы. Через
 * `toISOString()` ключ разъезжался бы сам с собой: банк выгружает одну и ту же
 * операцию то с временем (14:00 местного → UTC-день тот же), то без времени
 * (00:00 местного → UTC-день ПРЕДЫДУЩИЙ), и повторный импорт создавал бы
 * дубли — ровно то, ради чего ключ и заведён.
 *
