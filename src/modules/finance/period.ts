 ...
 * 1 июля 02:00 МСК уезжает в июнь, а итог не сходится с выпиской. Здесь
 * границы режутся по московским суткам, как и всё остальное в системе.
 *
...
function monthLabel(start: Date): string {
  // start — граница месяца по Москве, выраженная в UTC (для России это
  // 21:00 предыдущего дня). Ярлык обязан читаться по времени владельца, иначе
...
export function explicitRange(from: string, to: string): Range | null {
  ...
  // этих суток по Москве, иначе последний день молча выпадает из отчёта.
