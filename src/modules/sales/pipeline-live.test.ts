...
describe.runIf(live)("воронка на живой базе", () => {
  ...
  it("склейка не затирает заполненные поля свежим источником", async () => {
    // «Дописываем только пустое» — это про текущее значение поля, а не про
    // наличие входного. Писать всё, что пришло, значит молча терять правку
    // владельца: у справочника и адрес бывает старый, и город указан по
    // юрлицу, а не по месту работы.
    ...
    const stored = await prisma.lead.findUniqueOrThrow({ where: { id: first.leadId } });
    expect(stored.city).toBe("Первоуральск");
    expect(stored.address).toBe("Ватутина, 12");
    // Пустое всё так же дописывается — иначе склейка стала бы бесполезной.
    expect(stored.site).toBe("https://example.ru");
  });
