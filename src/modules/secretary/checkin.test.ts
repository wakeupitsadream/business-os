...
describe("дата чек-ина считается по времени владельца", () => {
  it("вечер — обычный случай, даты совпадают", () => {
    // 23:30 местного = 18:30 UTC того же дня.
    expect(asDay(checkInDate(new Date("2026-07-29T18:30:00Z")))).toBe("2026-07-29");
  });

  it("после местной полуночи наивный расчёт по UTC отнёс бы день к прошлому", () => {
    // Ровно та дыра, ради которой функция существует: с 00:00 до 04:59 местного
    ...
    const afterMidnightMsk = new Date("2026-07-29T21:30:00Z"); // 02:30 местного 30 июля
    expect(asDay(checkInDate(afterMidnightMsk))).toBe("2026-07-30");
    expect(naiveUtcDate(afterMidnightMsk)).toBe("2026-07-29");
  });

  it("граница держится до 04:59 местного", () => {
    const late = new Date("2026-07-29T23:59:00Z"); // 04:59 местного 30 июля
    expect(asDay(checkInDate(late))).toBe("2026-07-30");
    expect(naiveUtcDate(late)).toBe("2026-07-29");
  });

  it("после 05:00 местного расхождения больше нет", () => {
