...
describe("запись во время похода в базу не теряется", () => {
  ...
  it("устаревший ответ не сбрасывает отсчёт до сверки", () => {
    // NOW = 12:00 — слот 12:00–18:00. Свежая сверка была в нём, устаревший
    // ответ приходит там же, а проверяем уже в следующем слоте.
    setEarliestReminder(T("2026-12-31T00:00:00Z"), NOW);
    const stale = beginCursorSync();
    beginCursorSync();

    setEarliestReminder(T("2026-12-31T00:00:00Z"), T("2026-07-30T17:59:00Z"), stale);
    // Отсчёт ведётся от NOW, а не от времени устаревшего ответа: не будь этого,
    // устаревший ответ переставил бы отметку сверки и отодвинул следующую.
    expect(shouldCheckReminders(T("2026-07-30T18:00:00Z"))).toBe(true);
  });
...
describe("самолечение раз в час", () => {
  it("в следующем шестичасовом окне курсор перечитывается, даже если срок далеко", () => {
    // Страховка на случай, если новый путь записи забудут подключить к
    // курсору: цена забывчивости — задержка, а не потерянное напоминание.
    // Шесть часов вместо часа — размен на компьют, см. RESYNC_MS.
    setEarliestReminder(T("2026-12-31T00:00:00Z"), NOW);

    expect(shouldCheckReminders(T("2026-07-30T17:59:00Z"))).toBe(false);
    expect(shouldCheckReminders(T("2026-07-30T18:00:00Z"))).toBe(true);
  });

  it("окна привязаны к сетке от полуночи UTC, а не к моменту старта", () => {
    // Сетка, а не «шесть часов с прошлого раза». Иначе resync и суточный пульс
    // разъезжались бы по минутам и платили за два пробуждения компьюта вместо
    // одного, а расход зависел бы от минуты старта контейнера.
    setEarliestReminder(T("2026-12-31T00:00:00Z"), T("2026-07-30T17:30:00Z"));

    // До границы окна — не ходим, сразу после — ходим.
    expect(shouldCheckReminders(T("2026-07-30T17:59:00Z"))).toBe(false);
    expect(shouldCheckReminders(T("2026-07-30T18:00:00Z"))).toBe(true);
  });

  it("пустой список тоже перечитывается в новом окне", () => {
    setEarliestReminder(null, NOW);
    expect(shouldCheckReminders(T("2026-07-30T18:00:00Z"))).toBe(true);
  });
});
...
describe("экономия, ради которой всё затевалось", () => {
  it("сутки без напоминаний — четыре похода в базу вместо 1440", () => {
    let checks = 0;
    setEarliestReminder(null, NOW);
    ...
    // Было 24 — по одному на час. Каждое изолированное пробуждение компьюта
    // Neon стоит около пяти минут, то есть часовой resync съедал порядка
    // 15 CU-часов в месяц из ста общих с Agentus, не делая никакой работы.
