...
/**
 * Время в системе: хранение — UTC, бизнес-периоды — в поясе владельца.
 *
 * Это не педантизм: «вчерашние финансы» в утреннем брифе и месячные отчёты
 * режутся по суткам владельца. Операция в 02:00 по местному времени 1 августа
 * по UTC ещё июльская — без явной таймзоны она попала бы не в тот месяц и итог
 * не сошёлся бы с банковской выпиской.
 *
 * Пояс задаётся переменной окружения, а не зашит в код: переезд владельца не
 * должен требовать правки исходников и релиза. По умолчанию — Екатеринбург.
 *
 * Читается один раз при загрузке модуля. Смена пояса на живом приложении
 * означает перезапуск контейнера — и это правильно: половина процесса,
 * считающая сутки по-старому, была бы хуже минуты простоя.
 */

export const OWNER_TZ = process.env.OWNER_TZ?.trim() || "Asia/Yekaterinburg";
