...
describe("clientIp", () => {
  it("берёт адрес, записанный доверенным прокси, а не первый в цепочке", () => {
    // Первый элемент клиент подставляет сам: он присылает свой X-Forwarded-For,
    // а прокси лишь дописывает реальный адрес СПРАВА. Отсчёт справа — то, что
    // отличает настоящий адрес от заявленного.
...
describe("clientIp: устойчивость к подмене заголовка", () => {
  const hdr = (xff: string) => new Headers({ "x-forwarded-for": xff });

  it("берёт адрес, дописанный нашим прокси, а не заявленный клиентом", () => {
    // Клиент пишет левую часть сам. Раньше бралась именно она — и лимит
    // попыток входа обходился подстановкой нового адреса на каждый запрос.
    expect(clientIp(hdr("1.2.3.4, 203.0.113.7"))).toBe("203.0.113.7");
  });

  it("подделка не даёт разных ключей при одном реальном адресе", () => {
    const a = clientIp(hdr("10.0.0.1, 203.0.113.7"));
    const b = clientIp(hdr("10.0.0.2, 203.0.113.7"));
    expect(a).toBe(b);
  });

  it("одиночный адрес используется как есть", () => {
    expect(clientIp(hdr("203.0.113.7"))).toBe("203.0.113.7");
  });
